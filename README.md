...
- **农科院内网生产试点**：使用 `docker-compose.lan.yml`、`deploy/.env.production` 和 HTTPS 域名。共享服务器默认仅开放独立 HTTPS `8443`，数据库、API、文档解析和 Keycloak 均不直接暴露；获得专用反向代理后可切换至 `443`。
...
当前试用沙盒通过 CherryIn 的 OpenAI 兼容 API 调用模型，只接收公开或已脱敏数据。仅在服务器环境文件中填写 `LONGYUN_LLM_API_KEY`，不要写进前端、数据库、日志或 Git。旧版 `SHENNONG_*` 变量只作为迁移兼容项保留，新部署统一使用 `LONGYUN_LLM_*`。

旧聊天与四智能体共用同一模型外发策略：外部模型模式下，每次提交都必须确认本轮问题、附件和工具可读取的数据已经脱敏；未经确认不会发起模型请求。外部沙盒不会自动转发历史聊天或旧记忆，避免历史数据绕过本轮确认。

## 可信公开资料检索

联网检索默认关闭。只有同时设置 `TAVILY_API_KEY` 和 `LONGYUN_ALLOW_EXTERNAL_WEB_SEARCH=true`，并由用户完成本轮外发确认后，科研助手才会调用 Tavily Search API 补充可信公开来源。系统只向该服务发送由通用农业主题词组成的检索词，不发送附件正文。
...
预热后的模型缓存保存在本地 Docker 卷 `model_data`。常规 PDF、Office 文档和扫描件解析读取本地缓存，不会把附件上传给第三方解析服务。`bge-m3` 只对通过本地解析合格的知识库文本生成 1024 维向量，不会在用户请求中临时下载模型。图片附件不使用 Docling 或 PaddleOCR 进行本地文字解析；用户确认脱敏后，原图可能发送给当前配置的多模态模型服务，因此必须纳入同一沙盒脱敏规则。
...
- 数据处理员导入前选择管理员发布的正式模板。当前预置种质主数据、系谱关系、试验资料包、单株、基因型及文献情报模板；表型观测随试验资料包进入关联数据链路。
