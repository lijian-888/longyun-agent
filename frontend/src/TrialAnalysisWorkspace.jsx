...
export default function TrialAnalysisWorkspace({ onNotice }) {
  ...
  return <section className="intelligence-workspace">
    ...
    <section className="workspace-card trial-analysis-form"><label>已发布试验资料包<select value={form.package_id} onChange={(event) => setForm({ ...form, package_id: event.target.value, year: "", site_name: "", material_code: "", treatment_code: "" })}><option value="">请选择</option>{packages.map((item) => <option value={item.id} key={item.id}>{item.package_name} · {item.trial_count} 个试验 · {item.observation_count} 条观测</option>)}</select></label><label>分析方法<select value={form.analysis_type} onChange={(event) => setForm({ ...form, analysis_type: event.target.value })}>{METHODS.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label><div className="parameter-grid"><label>年份<select value={form.year} onChange={(event) => setForm({ ...form, year: event.target.value, site_name: "" })}><option value="">全部/请选择</option>{options.years.map((year) => <option value={year} key={year}>{year}</option>)}</select></label><label>地点<select value={form.site_name} onChange={(event) => setForm({ ...form, site_name: event.target.value })}><option value="">全部/请选择</option>{availableSites.map((item) => <option value={item.site_name} key={`${item.trial_year}-${item.site_code}`}>{item.trial_year} · {item.site_name}</option>)}</select></label><label>材料<select value={form.material_code} onChange={(event) => setForm({ ...form, material_code: event.target.value })}><option value="">全部/请选择</option>{options.materials.map((item) => <option value={item.material_code} key={item.material_code}>{item.material_code} · {item.material_name}{item.is_check ? "（对照）" : ""}</option>)}</select></label><label>处理<select value={form.treatment_code} onChange={(event) => setForm({ ...form, treatment_code: event.target.value })}><option value="">全部/请选择</option>{options.treatments.map((item) => <option value={item.treatment_code} key={item.treatment_code}>{item.treatment_code} · {item.treatment_name}</option>)}</select></label></div><button className="primary-button" disabled={!form.package_id || running} onClick={run}>{running ? <LoaderCircle className="spin" size={16} /> : <BarChart3 size={16} />}执行质量检查与分析</button></section>
