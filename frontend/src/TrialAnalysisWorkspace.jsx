...
export default function TrialAnalysisWorkspace({ onNotice }) {
  ...
  const [options, setOptions] = useState({ years: [], sites: [], materials: [], treatments: [] });
  ...
  useEffect(() => {
    if (!form.package_id) { setOptions({ years: [], sites: [], materials: [], treatments: [] }); return; }
    request(`/api/research/trial-analysis/packages/${form.package_id}/options`).then(setOptions).catch((requestError) => setError(requestError.message));
  }, [form.package_id]);
  useEffect(() => () => { if (chartUrl) URL.revokeObjectURL(chartUrl); }, [chartUrl]);

  const availableSites = options.sites.filter((item) => !form.year || String(item.trial_year) === String(form.year));
  ...
  return <section className="intelligence-workspace">
    ...
    <section className="workspace-card trial-analysis-form"><label>已发布试验资料包<select value={form.package_id} onChange={(event) => setForm({ ...form, package_id: event.target.value, year: "", site_name: "", material_code: "", treatment_code: "" })}><option value="">请选择</option>{packages.map((item) => <option value={item.id} key={item.id}>{item.package_name} · {item.trial_count} 个试验 · {item.observation_count} 条观测</option>)}</select></label><label>分析方法<select value={form.analysis_type} onChange={(event) => setForm({ ...form, analysis_type: event.target.value, year: "", site_name: "", material_code: "", treatment_code: "" })}>{METHODS.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label><div className="parameter-grid"><label>年份<select disabled={form.analysis_type !== "same_trial"} value={form.year} onChange={(event) => setForm({ ...form, year: event.target.value, site_name: "" })}><option value="">{form.analysis_type === "same_trial" ? "请选择" : "当前方法不使用"}</option>{options.years.map((year) => <option value={year} key={year}>{year}</option>)}</select></label><label>地点<select disabled={form.analysis_type !== "same_trial"} value={form.site_name} onChange={(event) => setForm({ ...form, site_name: event.target.value })}><option value="">{form.analysis_type === "same_trial" ? "请选择" : "当前方法不使用"}</option>{availableSites.map((item) => <option value={item.site_name} key={`${item.trial_year}-${item.site_code}`}>{item.trial_year} · {item.site_name}</option>)}</select></label><label>材料<select disabled={form.analysis_type !== "decline"} value={form.material_code} onChange={(event) => setForm({ ...form, material_code: event.target.value })}><option value="">{form.analysis_type === "decline" ? "请选择" : "当前方法分析全部材料"}</option>{options.materials.map((item) => <option value={item.material_code} key={item.material_code}>{item.material_code} · {item.material_name}{item.is_check ? "（对照）" : ""}</option>)}</select></label><label>处理<select disabled={!['same_trial', 'stability'].includes(form.analysis_type)} value={form.treatment_code} onChange={(event) => setForm({ ...form, treatment_code: event.target.value })}><option value="">{['same_trial', 'stability'].includes(form.analysis_type) ? "默认标准施氮" : "当前方法使用固定/全部处理"}</option>{options.treatments.map((item) => <option value={item.treatment_code} key={item.treatment_code}>{item.treatment_code} · {item.treatment_name}</option>)}</select></label></div><button className="primary-button" disabled={!form.package_id || running} onClick={run}>{running ? <LoaderCircle className="spin" size={16} /> : <BarChart3 size={16} />}执行质量检查与分析</button></section>
    {!packages.length && <div className="workspace-empty">当前课题没有已发布的区域试验资料包。请先在数据处理工作台导入并通过设计校验后发布。</div>}
    {result && <>
      <section className="workspace-card analysis-summary"><div><small>统计运行 {result.analysis_run_id} · {result.engine} · {result.analysis_version}</small><h3>{result.title}</h3><p>{result.model_formula}</p></div><div className="structured-result-actions"><button onClick={() => download(`/api/research/trial-analysis/runs/${result.analysis_run_id}/report.pdf`, "田间试验自动分析报告.pdf").catch((downloadError) => onNotice?.(downloadError.message))}><Download size={16} />分析报告</button><button onClick={() => download(`/api/research/trial-analysis/runs/${result.analysis_run_id}/ledger.xlsx`, "试验台账.xlsx").catch((downloadError) => onNotice?.(downloadError.message))}><Download size={16} />试验台账</button><button onClick={() => download(`/api/research/trial-analysis/runs/${result.analysis_run_id}/project-application-draft.pdf`, "课题申报辅助材料草稿.pdf").catch((downloadError) => onNotice?.(downloadError.message))}><Download size={16} />申报材料草稿</button></div></section>
