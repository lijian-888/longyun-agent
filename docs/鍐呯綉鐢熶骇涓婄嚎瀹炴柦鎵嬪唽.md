...
6. 确定敏感数据外发政策。神农 API 与 Tavily 是外部服务；未书面批准前，将 `SHENNONG_API_KEY`、`TAVILY_API_KEY` 留空，平台仍可使用本地结构化查询、知识库、文件解析和统计能力。
...
6. 若启用神农/Tavily，使用经批准的非敏感样例验证；再检查日志中不记录 API Key 与附件正文。
...
- 为神农、Tavily 建立“允许外发/禁止外发”的材料分级和审批策略；敏感材料应路由到后续本地大模型能力。
